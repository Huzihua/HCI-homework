    ...
    <div>
        <a-descriptions borderd size="default" title="订单详情" bordered layout="vertical">
            ...
            <a-descriptions-item label="订单状态" >
                ...
                <a-tag color="blue" v-else-if="currentOrderInfo.orderState=='已入住'">已入住</a-tag>
                ...
                <a-tag color="orange" v-else-if="currentOrderInfo.orderState=='待点评'">待点评</a-tag>
                <a-tag color="orange" v-else-if="currentOrderInfo.orderState=='已评价'">已评价</a-tag>
                <a-tag color="green" v-else>已预订</a-tag>
            </a-descriptions-item>

            <a-descriptions-item label="登记人数">
                {{currentOrderInfo.peopleNum}}人
            </a-descriptions-item>
            <a-descriptions-item label="房间信息">
                <span v-if="currentOrderInfo.roomType == 'BigBed'">大床房</span>
                <span v-if="currentOrderInfo.roomType == 'DoubleBed'">双床房</span>
                <span v-if="currentOrderInfo.roomType == 'Family'">家庭房</span>
                ：{{currentOrderInfo.roomNum}}间<br/>
            </a-descriptions-item>
            <a-descriptions-item label="有无儿童">
                {{Number(currentOrderInfo.haveChild)===0?'无':'有'}}<br/>
            </a-descriptions-item>
        </a-descriptions>
